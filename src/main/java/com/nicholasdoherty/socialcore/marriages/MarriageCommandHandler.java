	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (cmd.getName().equalsIgnoreCase("marriage")) {//show all marriage commands
				...
				int page = 0;
				List<String> allMarriages = sc.save.getAllMarriageNames();
				if (args.length == 1) {
				...
				if (player.hasPermission("sc.view.marriages")) {
					...
					if (upperbound-lowerbound > 0) {
						for (String s : allMarriages.subList(lowerbound,upperbound)) {
				...
				List<String> allEngagements = sc.save.getAllEngagements();
				int page = 0;
				if (args.length == 1) {
					page = Integer.valueOf(args[0])-1;
				}
				...
				if (player.hasPermission("sc.view.engagements")) {
					player.sendMessage(ChatColor.GOLD+"These are the marriages on the server: (Page " + (page+1)+")");
					if (upperbound-lowerbound > 0) {
						for (String s : allEngagements.subList(lowerbound,upperbound)) {
				...
				if (player.hasPermission("sc.view.divorces")) {
					List<String> allDivorces = sc.save.getAllDivorces();
					int page = 0;
					if (args.length == 1) {
						page = Integer.valueOf(args[0])-1;
					}
					...
					if (upperbound-lowerbound > 0) {
						for (String s : allDivorces.subList(lowerbound,upperbound)) {
							Engagement e = sc.save.getEngagement(s);
				...
				if (args.length < 1) {
					if (player.hasPermission("sc.fileDivorce")) {
						...
						if (sc.marriages.getPendingDivorces().contains(p1.getPlayerName())) {
                            ...
                            Case alreadyCase = Courts.getCourts().getDivorceManager().getCase(divorce);
				...
				double maxDistaceSquared = sc.lang.maxShareInventDistanceSquared;
