...
public class Race {
    ...
    private final String permissionToChose;
    private final String joinInfo;
    private final Map<String, Boolean> permissionsGiven;
    
    public Race(final String name, final String loreName, final String shortInfo, final String longInfo, final String permissionToChose, final Map<String, Boolean> permissionsGiven, final String joinInfo) {
        this.name = name;
    ...
    public void applyRace(final Player p) {
        final PermissionAttachment pa = (PermissionAttachment) p.getMetadata("pa").get(0).value();
        applyRace(p, pa);
    }
            ...
            public void run() {
                if(permissionsGiven != null) {
                    ...
                    for(String permission : permissionsGiven.keySet()) {
                        final boolean setTo;
                        final String origPermission = permission;
                        ...
                        if(permission.length() >= 1) {
                            if(!p.isPermissionSet(permission)) {
                                ...
                                final PermissionAttachmentInfo info = null;
    ...
    public void unapplyRace(final Player p) {
        final PermissionAttachment pa = (PermissionAttachment) p.getMetadata("pa").get(0).value();
		/*if (permissionsGiven != null) {
			for (String permission : permissionsGiven.keySet()) {
        ...
        for(final String permission : new HashSet<>(pa.getPermissions().keySet())) {
            pa.unsetPermission(permission);
        }
    }
