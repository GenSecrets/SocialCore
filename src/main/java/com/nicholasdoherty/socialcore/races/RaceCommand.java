...
public class RaceCommand implements CommandExecutor {
    ...
    public RaceCommand(final SocialCore plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String label, final String[] args) {
        ...
        if(p.getName().equalsIgnoreCase("jesusthecaffeine")) {
            System.out.println("spawn: " + p.getWorld().spawnEntity(p.getLocation(), EntityType.PIG));
        }
        if(args.length == 1) {
            ...
            if(subCommand.equalsIgnoreCase("join")) {
                ...
                final SocialPlayer socialPlayer = plugin.save.getSocialPlayer(p.getName());
        ...
        if(args.length == 2) {
            final String subCommand = args[0];
            if(subCommand.equalsIgnoreCase("s") || subCommand.equalsIgnoreCase("status")) {
                if(!p.hasPermission("sc.racestatus")) {
                    p.sendMessage(ChatColor.RED + "You do not have permission to use /race status");
                    return true;
                }
                final String toCheck = args[1];
                final SocialPlayer socialPlayer = plugin.save.getSocialPlayer(toCheck);
                if(socialPlayer == null) {
                    p.sendMessage(ChatColor.RED + "Could not get player: " + toCheck);
                } else {
                    p.sendMessage(ChatColor.GREEN + socialPlayer.getPlayerName() + ChatColor.RESET + " is of the race: " + ChatColor.translateAlternateColorCodes('&', socialPlayer.getRace().getLoreName()));
                }
                return true;
                ...
                final String raceString = args[1].toLowerCase();
                final Race race = plugin.races.getRaceByName(raceString);
                if(race == null) {
                ...
                if(race.getPermissionToChose() != null && !race.getPermissionToChose().isEmpty()) {
                ...
                final long secondsSinceLast = new Date().getTime() / 1000 - SocialCore.plugin.save.getSocialPlayer(p.getName()).getLastRaceChange();
                if(secondsSinceLast < plugin.races.cooldown) {
                    String message = plugin.races.commandStrings.get("join").get("cooldown");
                    message = message.replace("<race>", race.getLoreName());
                    final long seconds = plugin.races.cooldown - secondsSinceLast;
                    final String time = String.format("%d Hours %d Minutes, %d Seconds",
                ...
                final long curTime = new Date().getTime() / 1000;
                ...
                final String raceString = args[1].toLowerCase();
                final Race race = plugin.races.getRace(raceString);
                if(race == null) {
        ...
        if(command.getName().equalsIgnoreCase("races") && args.length <= 2 || command.getName().equalsIgnoreCase("race") && args.length >= 1 && args[0].equalsIgnoreCase("list")) {
            final List<Race> races = plugin.races.getRaces();
            final Map<String, String> listCommand = plugin.races.commandStrings.get("list");
            int page = 0;
            ...
            for(final Race race : races.subList(lowerbound, upperbound)) {
        ...
        for(final Map<String, String> map : plugin.races.commandStrings.values()) {
